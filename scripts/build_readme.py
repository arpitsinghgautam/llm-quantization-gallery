...
def render_timeline(methods):
    ...
    # Mermaid gantt timeline at the top
    gantt_lines = [
    ...
    for cat in CATEGORY_ORDER:
        meta = CATEGORY_META.get(cat, {})
        cat_methods = [m for m in sorted_methods if m.get("category") == cat]
        if not cat_methods:
            continue
        gantt_lines.append(f"    section {meta.get('abbr', cat)}")
        for m in cat_methods:
            ...
            gantt_lines.append(f"        {name_safe} :milestone, {d}, 0d")
    gantt_lines.append("```\n")
...
def main():
    ...
    # Chronological overview (plain markdown table, 2022–2025)
    readme_parts.append(render_chronological_section(methods))

    # Lineage graph → docs/lineage.md (not in README)
